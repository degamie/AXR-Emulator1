...
public class CameraPermissionHelper extends CameraManager {
    ...
    public String getCameraManager(CameraManager cameraManager){return cameraManager;}//Fetching CameraManager
...
//    public <CAMERA_PERMISSION_CODE> int OnInit(CAMERA_PERMISSION_CODE, CAMERA_PERMISSION){//OnInit method declare
public int OnInit(CAMERA_PERMISSION_CODE, CAMERA_PERMISSION){//OnInit method declare
        if(CAMERA_PERMISSION_CODE==0)return 0;
        while(CAMERA_PERMISSION_CODE!=0) {
            else if (CAMERA_PERMISSION_CODE > 0)
                CAMERA_PERMISSION_CODE == CAMERA_PERMISSION.getBytes(CameraId);
   ...
   return CAMERA_PERMISSION_CODE;
    ...
    public static boolean CameraOpenSafe(Activity activity, String cameraId){
        boolean isCameraOpen=false;
        try{
            releaseCameraAndPreview();
            Camera camera=camera.IsCameraOpen(cameraId);
            releaseCameraAndPreview();
            Camera camera=camera.getLocationX(cameraId);
            Camera camera=camera.getLocationY(cameraId);
